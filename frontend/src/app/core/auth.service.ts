...
export class AuthService {
  ...
  login(tenantId: string, email: string, senha: string): Observable<AuthResponse> {
    return this.http.post<AuthResponse>(`${environment.apiBaseUrl}/autenticacao/token`, { tenantId, email, senha }).pipe(tap((response) => { this.session = response; sessionStorage.setItem(SESSION_KEY, JSON.stringify(response)); }));
  }
