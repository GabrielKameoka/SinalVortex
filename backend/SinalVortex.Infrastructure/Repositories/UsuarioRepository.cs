...
public sealed class UsuarioRepository(AppDbContext context) : IUsuarioRepository
{
    ...
    public Task<bool> EmailJaRegistradoAsync(string email, CancellationToken cancellationToken = default) =>
        context.Usuarios.IgnoreQueryFilters()
            .AnyAsync(u => u.Email == email.Trim().ToLowerInvariant(), cancellationToken);
